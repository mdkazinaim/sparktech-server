...
import { visitorSessionSchema } from "./visitor.model";
...
const geolocate = async (req: Request, ip: string) => {
...
const saveHeartbeat = async (
  ...
  const VisitorSession = getTenantModel(req, "VisitorSession", visitorSessionSchema);
  const now = new Date();

  // Find existing session
  let session = await VisitorSession.findOne({ sessionToken: payload.sessionToken });
...
const getVisitorStats = async (req: Request) => {
  const VisitorSession = getTenantModel(req, "VisitorSession", visitorSessionSchema);
  const now = new Date();
  
  // 1. Online Now criteria (active within last 2 minutes)
  const twoMinutesAgo = new Date(now.getTime() - 2 * 60 * 1000);
  const onlineSessions = await VisitorSession.find({
  ...
  const total24hSessions = await VisitorSession.find({
  ...
  const countryAggregation = await VisitorSession.aggregate([
  ...
  const pageAggregation = await VisitorSession.aggregate([
    { $match: { updatedAt: { $gte: twentyFourHoursAgo } } },
    { $unwind: "$pagesVisited" },
